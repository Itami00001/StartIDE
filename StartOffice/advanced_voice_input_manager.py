...
class AdvancedVoiceInputManager:
    ...
    def __init__(self):
        ...
        # Настройки для русского языка — оптимизированные
        self.language = "ru-RU"
        self.energy_threshold = 250          # Чувствительнее к тихой речи
        self.pause_threshold = 0.5           # Короче пауза → быстрее реакция
        self.phrase_timeout = 3              # Ожидание фразы
        self.confidence_threshold = 0.5     # Ниже порог → больше распознаётся
        self.dynamic_energy = True          # Авторегулировка громкости
    ...
    def init_voice_input(self) -> bool:
        """Инициализация микрофона с оптимизированными настройками"""
        try:
            self.microphone = sr.Microphone()

            with self.microphone as source:
                # Калибровка под окружающий шум (1 секунда)
                ...
                self.recognizer.non_speaking_duration = 0.3   # Быстрее детектирует паузу
                self.recognizer.operation_timeout = None       # Без timeout на распознавание

            self.logger.info(
                f"Микрофон инициализирован. Порог энергии: {self.recognizer.energy_threshold:.0f}"
            )
            return True
    ...
    def _continuous_document_loop(self):
        """Цикл непрерывного распознавания — оптимизированный"""
        while self.is_recording:
            try:
                with self.microphone as source:
                    # phrase_time_limit=6 — даёт время произнести целую фразу
                ...
                if self.is_recording and audio:
                    # Обрабатываем в отдельном потоке, чтобы не блокировать запись
                    threading.Thread(
                        target=self._process_document_audio,
                        args=(audio,),
                        daemon=True
                    ).start()

            except sr.WaitTimeoutError:
                # Тишина — нормально, продолжаем слушать
                continue
            except OSError as e:
                # Проблема с микрофоном
                self.logger.error(f"Ошибка микрофона: {e}")
                break
            except Exception as e:
                self.logger.debug(f"Ошибка в цикле: {e}")
                time.sleep(0.05)
    ...
    def _recognize_with_optimization(self, audio_data) -> str:
        """Распознавание речи — Google STT с лучшими альтернативами"""
        # Попытка 1: Google с show_all=True для выбора лучшего варианта
        try:
            ...
            if isinstance(result, dict) and result.get('alternative'):
                # Берём вариант с наибольшей уверенностью
                alternatives = result['alternative']
                best = max(alternatives, key=lambda x: x.get('confidence', 0))
                confidence = best.get('confidence', 0)
                text = best.get('transcript', '')
                if text and confidence >= self.confidence_threshold:
                    return text.strip()
                # Если лучший вариант ниже порога, берём всё равно (не молчать)
                if text:
                    return text.strip()
            elif isinstance(result, str) and result:
                return result.strip()
        except sr.UnknownValueError:
            pass  # Тишина или неразборчиво
        except sr.RequestError as e:
            self.logger.warning(f"Google STT недоступен: {e}")

        # Попытка 2: Google без show_all (запасной вариант)
        try:
