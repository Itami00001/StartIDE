...
class StartOffice:
    ...
    def setup_overview_tab(self):
        ...
        # Git информация
        git_frame = ttk.LabelFrame(self.overview_frame, text="🌿 Git информация")
        git_frame.pack(fill=tk.X, padx=10, pady=10)

        self.git_branch_label = ttk.Label(git_frame, text="Ветка: -")
        self.git_branch_label.pack(anchor=tk.W, padx=10, pady=2)

        self.git_status_label = ttk.Label(git_frame, text="Статус: -")
        self.git_status_label.pack(anchor=tk.W, padx=10, pady=2)

        self.git_commits_label = ttk.Label(git_frame, text="Коммитов: -")
        self.git_commits_label.pack(anchor=tk.W, padx=10, pady=2)

        self.git_remote_label = ttk.Label(git_frame, text="Удаленный: -")
        self.git_remote_label.pack(anchor=tk.W, padx=10, pady=2)
    ...
    def discover_projects(self):
        ...
        try:
            ...
            for proj in projects:
                project_dict = {
                    'id': proj[0] if isinstance(proj, (list, tuple)) else proj.get('id'),
                    'name': proj[1] if isinstance(proj, (list, tuple)) else proj.get('name'),
                    'path': proj[2] if isinstance(proj, (list, tuple)) else proj.get('path'),
                }
                self.projects.append(project_dict)
                self.project_listbox.insert(tk.END, project_dict['name'])
        except Exception as e:
            self.logger.error(f"Ошибка загрузки проектов: {e}")

        self.status_bar.config(text=f"Проектов в базе: {len(self.projects)}")
    ...
    def new_project(self):
        ...
        try:
            ...
            if dialog.result:
                project_path = dialog.result.get('path', '')
                project_name = dialog.result.get('name', '')
                if project_path:
                    project_id = self.db_manager.get_or_create_project(project_path)
                else:
                    project_id = None
                if project_id:
                    self.discover_projects()
    ...
    def open_project(self):
        ...
        try:
            ...
            self.project_name_label.config(text=f"Проект: {project_path.name}")
    ...
    def update_status_indicators(self):
        ...
        try:
            ...
            # Индикатор голосового ввода
            if hasattr(self, 'advanced_voice_input_manager') and self.advanced_voice_input_manager:
                if hasattr(self, 'is_recording_document_voice') and self.is_recording_document_voice:
    ...
    def _display_project_info(self, project_data: Dict):
        ...
        try:
            # Обновляем основные метки проекта
    ...
    def update_project_statistics(self):
        ...
        try:
            ...
            # Обновляем историю текст
            if hasattr(self, 'history_text'):
                self.history_text.config(state=tk.NORMAL)
                self.history_text.delete("1.0", tk.END)
                self.history_text.insert("1.0", stats_text)
    ...
    def on_project_select(self, event):
        ...
        if selection:
            ...
            self.current_project_id = self.db_manager.get_or_create_project(project_path) if project_path else project.get('id')
    ...
    def load_project_details(self, project_path):
        """Загрузка деталей проекта из файловой системы и БД"""
        try:
            self.current_project_path = project_path
            ...
            # Подсчёт файлов и папок напрямую из ФС
            try:
                files = [f for f in project_path_obj.rglob('*') if f.is_file() and not any(p.startswith('.') for p in f.parts)]
                folders = [f for f in project_path_obj.iterdir() if f.is_dir() and not f.name.startswith('.')]
                self.project_files_label.config(text=f"Файлов: {len(files)}")
                self.project_folders_label.config(text=f"Папок: {len(folders)}")
            except Exception:
                self.project_files_label.config(text="Файлов: -")
                self.project_folders_label.config(text="Папок: -")

            # История из чата в БД
            if self.current_project_id and self.chat_manager:
                msgs = self.chat_manager.get_messages(self.current_project_id, limit=10)
                history = [{'timestamp': m.get('timestamp',''), 'action': m.get('sender',''), 'details': m.get('content','')[:80]} for m in msgs]
            ...
            try:
                ...
                for item in project_path_obj.iterdir():
                    if item.is_dir() and not item.name.startswith('.'):
                        structure['folders'].append({'name': item.name})
                    elif item.is_file():
                        structure['files'].append({'name': item.name, 'size': item.stat().st_size})
                self.update_structure_display(structure)
            except Exception:
                pass
        ...
        if self.ollama_manager:
            def get_ai_response():
                try:
                    ...
                    # Отправляем запрос в AI с контекстом
...
Ответ на русском языке."""
    ...
    def update_git_info_display(self):
        """Обновление отображения Git информации"""
        try:
            if not hasattr(self, 'git_manager') or self.git_manager is None or not self.git_manager.is_git_repo:
                self.git_branch_label.config(text="Ветка: Git репозиторий не найден")
                self.git_status_label.config(text="Статус: -")
                self.git_commits_label.config(text="Коммитов: -")
                self.git_remote_label.config(text="Удаленный: -")
                return

            # Текущая ветка
            current_branch = self.git_manager.get_current_branch()
            self.git_branch_label.config(text=f"Ветка: {current_branch}")

            # Статус
            status = self.git_manager.get_git_status()
            status_text = "Чистый" if status['is_clean'] else "Есть изменения"
            if not status['is_clean']:
                status_text += f" (📝{len(status['modified'])} ➕{len(status['untracked'])})"
            self.git_status_label.config(text=f"Статус: {status_text}")

            # Количество коммитов
            commits = self.git_manager.get_commits(1000)  # Получаем все коммиты
            self.git_commits_label.config(text=f"Коммитов: {len(commits)}")

            # Удаленный репозиторий
            remote_url = self.git_manager.get_remote_url()
            if remote_url:
                # Короткое имя удаленного репозитория
                remote_name = remote_url.split('/')[-1].replace('.git', '') if '/' in remote_url else remote_url
                self.git_remote_label.config(text=f"Удаленный: {remote_name}")
            else:
                self.git_remote_label.config(text="Удаленный: -")

            # Обновляем историю изменений
            self.update_git_history(commits[:10])  # Последние 10 коммитов

        except Exception as e:
            self.logger.error(f"Ошибка обновления Git информации: {e}")

    def update_git_history(self, commits):
        """Обновить историю Git изменений"""
        try:
            self.history_text.config(state=tk.NORMAL)
            self.history_text.delete("1.0", tk.END)

            if not commits:
                self.history_text.insert("1.0", "Нет коммитов в репозитории")
            else:
                history_text = "📜 Последние коммиты:\n\n"
                for i, commit in enumerate(commits[:10], 1):
                    commit_hash = commit['commit_hash'][:8]
                    commit_msg = commit['message'][:60] + "..." if len(commit['message']) > 60 else commit['message']
                    commit_author = commit['author']
                    commit_date = commit['date']

                    history_text += f"{i}. {commit_hash} - {commit_msg}\n"
                    history_text += f"   👤 {commit_author} 📅 {commit_date}\n\n"
            ...
            self.logger.error(f"Ошибка обновления истории Git: {e}")

    def get_project_context_from_db(self):
        """Получение контекста проекта из базы данных"""
        try:
            if not self.current_project_id:
                return None
            ...
            cursor = self.db_manager.conn.execute(
                "SELECT name, path, description FROM projects WHERE id = ?",
                (self.current_project_id,)
            )
            project = cursor.fetchone()
            ...
            # Формируем контекст
            context_lines = []
            context_lines.append(f"=== КОНТЕКСТ ПРОЕКТА ===")
            context_lines.append(f"Название: {project['name']}")
            context_lines.append(f"Путь: {project['path']}")
            if project['description']:
                context_lines.append(f"Описание: {project['description']}")
            context_lines.append("")

            # Добавляем технологический стек
            try:
                cursor = self.db_manager.conn.execute(
                    "SELECT technology, version, confidence FROM tech_stack WHERE project_id = ? ORDER BY confidence DESC",
                    (self.current_project_id,)
                )
            ...
            if tech_stack:
                context_lines.append("=== ТЕХНОЛОГИЧЕСКИЙ СТАК ===")
                for tech in tech_stack:
                    version_info = f" ({tech['version']})" if tech['version'] else ""
                    context_lines.append(f"- {tech['technology']}{version_info} [уверенность: {tech['confidence']}]")
                context_lines.append("")

            # Добавляем последние сообщения чата
            cursor = self.db_manager.conn.execute(
                "SELECT sender, content, created FROM chat_messages WHERE project_id = ? ORDER BY created DESC LIMIT 10",
                (self.current_project_id,)
            )
            messages = cursor.fetchall()

            if messages:
                context_lines.append("=== ПОСЛЕДНИЕ СООБЩЕНИЯ В ЧАТЕ ===")
                for msg in reversed(messages):
                    sender = "Пользователь" if msg['sender'] in ['StartIDE', 'StartOffice'] else "AI"
                    context_lines.append(f"{sender}: {msg['content']}")
                context_lines.append("")

            # Добавляем Git информацию
            if hasattr(self, 'git_manager') and self.git_manager.is_git_repo:
                context_lines.append("=== GIT ИНФОРМАЦИЯ ===")
                branch = self.git_manager.get_current_branch()
                context_lines.append(f"Текущая ветка: {branch}")

                status = self.git_manager.get_git_status()
                if not status['is_clean']:
                    context_lines.append(f"Измененные файлы: {len(status['modified'])}")
                    context_lines.append(f"Новые файлы: {len(status['untracked'])}")
                else:
                    context_lines.append("Статус: Чистый репозиторий")

                # Последние коммиты
                commits = self.git_manager.get_commits(5)
                if commits:
                    context_lines.append("Последние коммиты:")
                    for commit in commits:
                        context_lines.append(f"- {commit['commit_hash'][:8]}: {commit['message'][:50]}...")

            return '\n'.join(context_lines)

        except Exception as e:
            self.logger.error(f"Ошибка получения контекста из БД: {e}")
            return None

    def send_context_to_neuro(self):
        """Отправка контекста проекта в нейросеть"""
        try:
            if not self.ollama_manager:
                return
            ...
            # Отправляем в Ollama для анализа
            response = self.ollama_manager.send_project_context(
                project_name=self.project_name_label.cget("text").replace("Проект: ", ""),
                context_content=context_content
            )

            if response:
                self.logger.info("Контекст успешно отправлен в нейросеть")

        except Exception as e:
            self.logger.error(f"Ошибка отправки контекста в нейросеть: {e}")
