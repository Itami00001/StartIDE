...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        try:
            ...
            # Всегда создаём таблицы (безопасно: используется IF NOT EXISTS)
            self._create_tables_manually()
            self.logger.info("Таблицы базы данных проверены/созданы")
    ...
    def create_project(self, name: str, path: str, description: str = "", tech_stack=None) -> Optional[int]:
        """Создание нового проекта (псевдоним для get_or_create_project с расширенными данными)"""
        try:
            project_path = str(Path(path).resolve())
            # Проверяем существующий проект
            cursor = self.conn.execute("SELECT id FROM projects WHERE path = ?", (project_path,))
            existing = cursor.fetchone()
            if existing:
                return existing[0]
            # Создаём новый
            project_name = name or Path(project_path).name
            self.conn.execute(
                "INSERT INTO projects (name, path, status) VALUES (?, ?, 'active')",
                (project_name, project_path)
            )
            self.conn.commit()
            cursor = self.conn.execute("SELECT id FROM projects WHERE path = ?", (project_path,))
