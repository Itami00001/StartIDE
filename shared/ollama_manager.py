...
class OllamaManager:
    ...
    def send_chat_message(self, message: str, chat_context: str = "") -> Optional[str]:
        """Отправка сообщения в AI-чат с сохранением контекста диалога"""
        try:
            prompt_parts = [
                "Ты — AI-ассистент для разработки программного обеспечения.",
                "Отвечай на русском языке. Будь кратким и точным.",
                ""
            ]
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"Ошибка Ollama: {response.status_code}")
                return None
    ...
    def ask_question(self, prompt: str) -> Optional[str]:
        """Задать произвольный вопрос нейросети"""
        try:
            response = self.session.post(
                f"{self.base_url}/api/generate",
                json={
